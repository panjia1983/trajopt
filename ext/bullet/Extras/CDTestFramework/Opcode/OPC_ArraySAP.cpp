...
inline_ void Sort(uword& id0, uword& id1, const void*& obj0, const void*& obj1)
{
  if(id0 > id1)
  {
    TSwap(id0, id1);
...
struct Opcode::IAABB : public Allocateable
{
  ...
  inline_ udword	GetMin(udword i)	const
  {
    return (&mMinX)[i];
  }
  inline_ udword	GetMax(udword i)	const
  {
    return (&mMaxX)[i];
  }
};
...
inline_ bool DifferentPair(const ASAP_Pair& p, uword id0, uword id1)
{
  return (id0 != p.id0) || (id1 != p.id1);
}
...
void ASAP_PairManager::ReallocPairs()
{
  ...
  ASAP_Pair* NewPairs	= (ASAP_Pair*)ICE_ALLOC(mHashSize * sizeof(ASAP_Pair));
  ASSERT(NewPairs);
  udword* NewNext		= (udword*)ICE_ALLOC(mHashSize * sizeof(udword));
  ASSERT(NewNext);
...
public:
  ...
  inline_	bool			IsSentinel()	const
  {
    return (mData&~3) == 0xfffffffc;
  }
...
public:
  ...
  inline_	void			SetInvalid()
  {
    mMin[0] = INVALID_INDEX;
  }
  inline_	bool			IsValid()		const
  {
    return mMin[0] != INVALID_INDEX;
  }
