...
void    GLUI_EditText::deactivate(void)
{
  ...
  else if(data_type == GLUI_EDITTEXT_TEXT)
  {
...
void    GLUI_EditText::draw(int x, int y)
{
  ...
  glBegin(GL_LINES);
  glColor3f(.5, .5, .5);
  glVertex2i(text_x_offset, 0);
  glVertex2i(w, 0);
  glVertex2i(text_x_offset, 0);
  glVertex2i(text_x_offset, h);
  ...
  if(enabled)
    ...
    glColor3f(.25, .25, .25);
  glVertex2i(text_x_offset + 1, 1);
  glVertex2i(w - 1, 1);
  glVertex2i(text_x_offset + 1, 1);
  glVertex2i(text_x_offset + 1, h - 1);
...
void    GLUI_EditText::draw_text(int x, int y)
{
  ...
  if(NOT draw_text_only)
  {
    ...
    glBegin(GL_QUADS);
    glVertex2i(text_x_offset + 2, 2);
    glVertex2i(w - 2, 2);
    glVertex2i(w - 2, h - 2);
    glVertex2i(text_x_offset + 2, h - 2);
    glEnd();
  }
  ...
  if(sel_start != sel_end)
  {
    ...
    glBegin(GL_QUADS);
    glVertex2i(sel_x_start, 2);
    glVertex2i(sel_x_end, 2);
    glVertex2i(sel_x_end, h - 2);
    glVertex2i(sel_x_start, h - 2);
    glEnd();
  }
