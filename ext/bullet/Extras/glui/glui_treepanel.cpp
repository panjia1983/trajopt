...
void GLUI_TreePanel::formatNode(GLUI_Tree *temp)
{
  ...
  if(format & GLUI_TREEPANEL_ALTERNATE_COLOR)
  {
    ...
    {
    case(7):
      temp->set_color(.5, .5, .5);
      break;
    case(6):
      temp->set_color(.3, .5, .5);
      break;
    case(5):
      temp->set_color(.5, .3, .5);
      break;
    case(4):
      temp->set_color(.3, .3, .5);
      break;
    case(3):
      temp->set_color(.5, .5, .3);
      break;
    case(2):
      temp->set_color(.3, .5, .3);
      break;
    case(1):
      temp->set_color(.5, .3, .3);
      break;
    default:
      temp->set_color(.3, .3, .3);
    }
  }
  else
  {
  ...
  else
  {
    ...
    else if(format & GLUI_TREEPANEL_CONNECT_CHILDREN_ONLY)
    {
      temp->disable_bar();
