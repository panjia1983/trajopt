...
int Image::readBMP(FILE* fp, FILE* fp_a, bool bBaseImg)
{
  ...
  if(bBaseImg)
  {
    ...
    {
    case 1:
      channels = 1;			// 1 byte per bit - *bad storage, 2 color (B & W)
    case 4:
      channels = 1;			// 1 byte per 4-bits - not great, 16 color, index mode
    case 8:
      channels = 1;			// 1 byte per pixel, 256 color, index mode
    case 16:
      channels = 3;			// 2 byte per pixel, 16-bit color
    case 24:
      channels = 3;			// 3 byte per pixel (RGB), 24-bit color
    case 32:
      channels = 3;			// 4 byte per pixel, 32-bit color, compressed down to 24-bit
    }
...
int Image::writeBMP(FILE* fp)
{
  ...
  else
  {
    for(int y = 0; y < height; ++y)
    {
      ...
      {
        getPixel(x, height - y - 1, pixel);
        fputc((int)(pixel.b * 255), fp);
        nbytes++;
        fputc((int)(pixel.g * 255), fp);
        nbytes++;
        fputc((int)(pixel.r * 255), fp);
        nbytes++;
      }
