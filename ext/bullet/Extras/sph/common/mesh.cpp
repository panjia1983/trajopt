...
void Mesh::ComputeNormalsFVF()
{
  ...
  for(int n = 0; n < NumFace(); n++)
  {
    ...
    vn->ny += norm.y;
    vn->nz += norm.z;
    vn = (AttrNorm*)((char*) v2 + noff);
    vn->nx += norm.x;
    vn->ny += norm.y;
    vn->nz += norm.z;
    vn = (AttrNorm*)((char*) v3 + noff);
    vn->nx += norm.x;
    vn->ny += norm.y;
    vn->nz += norm.z;
    if(f->v4 != -1)
...
xref Mesh::AddFaceFast3CM(xref v1, xref v2, xref v3)
{
  ...
  f->v4 = -1;
  xref eNdx;
  eNdx = AddEdgeCM(f->v1, f->v2);
  f->e1 = eNdx;
  eNdx = AddEdgeCM(f->v2, f->v3);
  f->e2 = eNdx;
  eNdx = AddEdgeCM(f->v3, f->v1);
  f->e3 = eNdx;
...
xref Mesh::AddFaceFast4CM(xref v1, xref v2, xref v3, xref v4)
{
  ...
  eNdx = AddEdgeCM(f->v2, f->v3);
  f->e2 = eNdx;
  eNdx = AddEdgeCM(f->v3, f->v4);
  f->e3 = eNdx;
  eNdx = AddEdgeCM(f->v4, f->v1);
  f->e4 = eNdx;
  AddRef(fNdx, GetVertCM(v1)->flist, FACE_DELTA);
  AddRef(fNdx, GetVertCM(v2)->flist, FACE_DELTA);
  AddRef(fNdx, GetVertCM(v3)->flist, FACE_DELTA);
...
void Mesh::DrawGL(float* viewmat)
{
  ...
  case CM:
  {
    ...
    glColor3f(.6, .6, .6);
    DrawFacesCM(viewmat, 0, NumFace() - 1);
  ...
  case FVF:
  {
    ...
    glColor4f(.9, .9, .9, 0.75);
    DrawFacesFVF(viewmat, 0, NumFace() - 1);
...
void Mesh::Measure()
{
  ...
  vs = NumVert();
  if(vs != 0) vs *= GetStride(m_Vbuf);
  es = NumEdge();
  if(es != 0) es *= GetStride(m_Ebuf);
  fs = NumFace();
  if(fs != 0) fs *= GetStride(m_Fbuf);
