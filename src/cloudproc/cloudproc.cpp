...
template<class T>
vector< vector<int> > findClusters(typename pcl::PointCloud<T>::ConstPtr cloud, float tol, int minSize) {
...
template<class T>
typename pcl::PointCloud<T>::Ptr extractInds(typename pcl::PointCloud<T>::ConstPtr in, std::vector<int> inds) {
...
template<class T>
typename pcl::PointCloud<T>::Ptr  getBiggestCluster(typename pcl::PointCloud<T>::ConstPtr in, float tol) {
  vector< vector<int> > cluster_inds = findClusters<T>(in, tol, 10);
  if (cluster_inds.size() == 0) throw runtime_error("didn't find any clusters");
  Eigen::VectorXi sizes(cluster_inds.size());
  for (int i=0; i < cluster_inds.size(); i++) sizes(i) = cluster_inds[i].size();
  int iBest;
  sizes.maxCoeff(&iBest);
  cout << "sizes: " << sizes.transpose() << endl;
  cout << iBest << endl;
  return extractInds<T>(in, cluster_inds[iBest]);

}

template<class T>
typename pcl::PointCloud<T>::Ptr  clusterFilter(typename pcl::PointCloud<T>::ConstPtr in, float tol, int minSize) {
  vector< vector<int> > cluster_inds = findClusters<T>(in, tol, minSize);
  ...
  return extractInds<T>(in, filtered_cluster_inds);
}
...
template<class T>
typename pcl::PointCloud<T>::Ptr removeOutliers(typename pcl::PointCloud<T>::ConstPtr in, float thresh, int k) {
  typename pcl::PointCloud<T>::Ptr out (new typename pcl::PointCloud<T>);
