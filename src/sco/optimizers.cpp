...
void Optimizer::callCallbacks(DblVec& x, bool has_cleanup)
{
  // the id of steps (starting from 1)
  step_id_ ++;

  // the time since start (the first optimization step is 0, then increases)
  timer_.stop();
  time_since_start_ += timer_.getElapsedTimeInMilliSec();

  OptStepResults res;
  res.time_since_start = time_since_start_;
  res.step_id = step_id_;

  if(has_cleanup)
  {
    if(this->results_.status == OPT_CONVERGED)
      res.is_converged = OPTS_CONVERGED;
    else
      res.is_converged = OPTS_NOTCONVERGED;
  }
  
  for(int i = 0; i < callbacks_.size(); ++i)
  {
    callbacks_[i](prob_.get(), x, res);
  }
...
struct MultiCritFilter
{
  ...
  void insert(const DblVec& x)
  {
    errvecs.push_back(x);
  }
  bool empty()
  {
    return errvecs.size() > 0;
  }
};
#endif
