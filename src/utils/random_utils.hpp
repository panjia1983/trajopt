...
// fix the start and end configuration, randomly changing the other waypoints
inline MatrixXd randomTrajectory(const MatrixXd& init, const VectorXd& range, RNG& rng)
{
  MatrixXd newTraj = init;
  for(std::size_t i = 1; i < init.rows() - 1; ++i)
  {
    for(std::size_t j = 0; j < init.cols(); ++j)
    {
      double x = init(i, j) + rng.uniformReal(-1, 1) * range[j];
